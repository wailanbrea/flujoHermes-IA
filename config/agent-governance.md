...
5. **PROHIBIDO Mencionar URLs Rótulo/Uncompiled o Carpetas con Espacios**: Está estrictamente PROHIBIDO entregar URLs no compiladas o carpetas con espacios (ej. `http://localhost/Nombre Con Espacios/`) o sugerir abrir paneles de control manualmente (XAMPP, IIS, Docker). La ÚNICA URL válida para cualquier aplicación web DEBE ser un puerto HTTP limpio servido o compilado de forma autónoma (`http://127.0.0.1:8080`, `http://127.0.0.1:8000`, `http://127.0.0.1:5173`).
