...
function buildBrainWorkflow(
  ...
  const failureStates = new Set(["failed", "blocked", "validation-failed"]);
  const executionState: HealthState =
    execution.taskState && failureStates.has(execution.taskState)
      ? "degraded"
      : execution.mode === "idle"
        ? "unknown"
        : status;
  ...
  const stageIndexFor = (stageId: WorkflowStageId): number =>
    WORKFLOW_STAGES.findIndex((stage) => stage.id === stageId);
  const evidenceFor = (stageId: WorkflowStageId): WorkflowEdge["evidence"] => {
    const index = stageIndexFor(stageId);
    return index >= 0 && index <= observedThrough ? "observed" : "configured";
  };
  const stageState = (
    stageId: WorkflowStageId,
    currentState: HealthState = status,
  ): HealthState => {
    const index = stageIndexFor(stageId);
    if (index < 0 || execution.mode === "idle" || index > observedThrough) return "unknown";
    if (index < observedThrough) return "healthy";
    return execution.taskState && failureStates.has(execution.taskState)
      ? "degraded"
      : currentState;
  };

  // Layout lógico amplio. El frontend escala el lienzo únicamente cuando cabe sin
  // sacrificar legibilidad; en pantallas medianas conserva scroll interno.
  const MAIN_X = 120;
  const MAIN_W = 1240;
  const MAIN_H = 145;
  const BRANCH_W = 500;
  ...
  const nodes: WorkflowNode[] = [
    // FASE A — INGRESO Y CONTRATO
    {
      ...
      role: "Origen de la petición",
      detail: "Mensaje, comandos, archivos y metadatos del canal",
      purpose: "Entrega el objetivo original y los insumos que iniciarán una ejecución identificable.",
      state: telegramState,
      kind: "user",
      stageId: "input",
      stageNumber: 1,
    ...
    },
    {
      ...
      role: "Canal de ingress saneado",
      detail: "Gateway oficial de Hermes · Ingress autenticado",
      purpose: "Recibe la solicitud, asigna request_id y la entrega al pipeline sin capacidad de mutación directa.",
      state: telegramState,
      kind: "interface",
      stageId: "input",
      stageNumber: 1,
    ...
    },
    {
      ...
      role: "Control de identidad, permisos y duplicidad",
      detail: "Request UUID · Idempotency Key · Allowlist",
      purpose: "Impide ejecuciones duplicadas o no autorizadas antes de consumir herramientas y modelos.",
      state: stageState("auth"),
      kind: "policy",
      stageId: "auth",
      stageNumber: 2,
      phase: "ingress",
    ...
    },
    {
      ...
      role: "Especificación operativa verificable",
      detail: "Objetivo · Alcance · Restricciones · Criterios de aceptación",
      purpose: "Convierte lenguaje libre en un contrato de ejecución con entradas, entregables y condiciones de finalización.",
      state: stageState("normalize"),
      kind: "policy",
      stageId: "normalize",
      stageNumber: 3,
      phase: "ingress",
    ...
    },
    {
      ...
      role: "Evaluación inicial de complejidad y riesgo",
      detail: "Score 6D de 0–24 puntos · Elevación automática L4",
      purpose: "Determina cuánto contexto, planificación, modelo y validación requiere la solicitud antes de gastar VRAM.",
      state: stageState("preclassify"),
      kind: "router",
      stageId: "preclassify",
      stageNumber: 4,
      phase: "ingress",
    ...
    // FASE B — CONTEXTO Y ENRUTAMIENTO
    {
      ...
      purpose: "Recupera contexto estructural y gobierna las políticas que limitan la ejecución local.",
      state: status,
      kind: "control",
      stageId: "brain",
      stageNumber: 5,
    ...
    },
    {
      id: "trama",
      label: "TRAMA",
      role: "Observador lateral de solo lectura",
      detail: "SSE read-only · Telemetría sin control mutante",
      purpose: "Expone estados, métricas y evidencia saneada sin modificar código, prompts ni credenciales.",
      state: "healthy",
      kind: "observer",
      stageId: "brain",
      stageNumber: 5,
    ...
    },
    {
      ...
      role: "Recuperación estructural del código",
      detail: `${brain.memory.graphNodes.toLocaleString("es-DO")} nodos AST disponibles`,
      purpose: "Localiza archivos, símbolos, dependencias e impacto para evitar exploraciones indiscriminadas.",
    ...
    },
    {
      ...
      role: "Selección de modelo y presupuesto",
      detail: `${brain.router.routes.length} rutas · Gemma / Agents-A1 / Qwen`,
      purpose: "Elige capacidad, contexto y fallback según complejidad, herramientas y memoria gráfica disponible.",
    ...
    },
    {
      id: "agents",
      label: "Agent Factory",
      role: "Asignación de especialistas",
      detail: `${brain.agents.operatorCount} operador · ${brain.agents.advisoryCount} asesores`,
      purpose: "Activa los perfiles necesarios para analizar, implementar, revisar y probar la tarea.",
    ...
    // FASE C — PLANIFICACIÓN Y EJECUCIÓN
    {
      ...
      role: "Descomposición atómica y dependencias",
      detail: "Tareas T-001…T-025 · Criterios de aceptación · Repair target",
      purpose: "Convierte el objetivo en acciones verificables, ordenadas por dependencia y con evidencia esperada.",
      state: stageState("atomic-plan", status),
      kind: "policy",
      stageId: "atomic-plan",
      stageNumber: 7,
      phase: "execution",
    ...
    },
    {
      ...
      role: "Control de la siguiente tarea ejecutable",
      detail: "Allowlist de rutas · Task ID · Permisos · Scope",
      purpose: "Autoriza una tarea READY, limita su alcance y bloquea ramas con dependencias fallidas.",
    ...
    },
    {
      ...
      detail: `${brain.sandbox.active.length} tareas activas`,
      purpose: "Modifica y prueba archivos en un entorno temporal sin alterar la rama principal.",
      state: brain.sandbox.state,
      kind: "executor",
      stageId: "sandbox",
      stageNumber: 9,
    ...
    },
    {
      ...
      role: "Validación funcional en navegador",
      detail: "Capturas · Trazas · Consola · Network",
      purpose: "Comprueba flujos reales, responsive, errores de consola y resultados visibles.",
      state: brain.sandbox.state,
      kind: "executor",
      stageId: "sandbox",
      stageNumber: 9,
    ...
    },
    {
      ...
      role: "Operaciones explícitas y acotadas",
      detail: "Comandos permitidos · Logs · Salidas",
      purpose: "Ejecuta operaciones auxiliares bajo autorización y conserva una evidencia reproducible.",
      state: brain.sandbox.state,
      kind: "executor",
      stageId: "sandbox",
      stageNumber: 9,
    ...
    },
    {
      ...
      role: "Inspección del cambio y su prueba",
      detail: "Diff · SHA-256 · Límites · Tests · Capturas",
      purpose: "Rechaza cambios fuera de alcance y registra evidencia antes de permitir integración.",
      state: stageState("evidence", executionState),
      kind: "validation",
      stageId: "evidence",
      stageNumber: 10,
      phase: "execution",
    ...
    // FASE D — INTEGRACIÓN, CALIDAD Y REPARACIÓN
    {
      ...
      role: "Composición coherente del resultado",
      detail: "Contratos · Esquemas · Imports · Compatibilidad",
      purpose: "Combina los parches y confirma que el conjunto funciona como una sola solución.",
      state: stageState("integration", executionState),
      kind: "result",
      stageId: "integration",
      stageNumber: 11,
      phase: "quality",
    ...
    },
    {
      ...
      role: "Compilación, análisis y pruebas objetivas",
      detail: "Typecheck · Linters · Tests · Playwright · Criterios",
      purpose: "Ejecuta controles reproducibles y diferencia PASS, FAIL, NOT_AVAILABLE y NOT_APPLICABLE.",
      state: stageState("validated", executionState),
      kind: "result",
      stageId: "validated",
      stageNumber: 12,
      phase: "quality",
    ...
    },
    {
      ...
      role: "Búsqueda activa de errores y omisiones",
      detail: "Requisitos · Seguridad · Datos · UX · Regresión",
      purpose: "Busca fallos que los Quality Gates normales podrían no detectar y decide entrega o reparación.",
      state: stageState("audit", executionState),
      kind: "validation",
      stageId: "audit",
      stageNumber: 13,
      phase: "quality",
    ...
    },
    {
      ...
      role: "Corrección de causa raíz",
      detail: "Fix Tasks · Máx. 5 estrategias · Replanificación",
      purpose: "Convierte hallazgos en tareas FIX y retorna por un corredor lateral al Atomic Planner.",
      state: stageState("repair", executionState),
      kind: "validation",
      stageId: "repair",
      stageNumber: 14,
      phase: "quality",
    ...
    // FASE E — APRENDIZAJE Y ENTREGA
    {
      ...
      role: "Aprendizaje únicamente desde evidencia validada",
      detail: `${brain.learning.counts.candidate ?? 0} candidatas saneadas`,
      purpose: "Extrae patrones reutilizables y elimina secretos, datos temporales e hipótesis no confirmadas.",
    ...
    },
    {
      ...
      role: "Publicación del estado final real",
      detail: "Resultado estable · Memoria autorizada · Entrega",
      purpose: "Promueve únicamente resultados verificables y comunica archivos, pruebas, errores y limitaciones.",
  ...
  return {
    ...
    edges: [
      link("user", "telegram", "Solicitud recibida", "input", telegramState),
      link("telegram", "auth", "Ingress autenticado", "auth", telegramState),
