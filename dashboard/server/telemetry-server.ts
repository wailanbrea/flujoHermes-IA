...
function buildBrainWorkflow(
  ...
  const nodes: WorkflowNode[] = [
    { id: "user", label: "Usuario", role: "Solicitud", detail: "Objetivo y requerimientos", state: "healthy", kind: "user", stageId: "input", x: 390, y: 10, width: 220, height: 36 },
    { id: "telegram", label: "Telegram Gateway", role: "Canal de Ingress", detail: "Autenticación e Idempotencia", state: telegramState, kind: "interface", stageId: "input", x: 390, y: 58, width: 220, height: 38 },
    { id: "auth", label: "Autenticación & Idempotencia", role: "Filtro de duplicidad", detail: "Request UUID e Idempotency Key", state: "healthy", kind: "policy", stageId: "auth", x: 365, y: 108, width: 270, height: 38 },
    { id: "normalize", label: "Normalizador & Contrato", role: "Especificación de tarea", detail: "Alcance, restricciones y criterios", state: "healthy", kind: "policy", stageId: "normalize", x: 350, y: 158, width: 300, height: 40 },
    { id: "preclassify", label: "Clasificador Preliminar", role: "Niveles L0 a L4", detail: "Evaluación 6D (0-24 pts) / Elevación L4", state: "healthy", kind: "router", stageId: "preclassify", x: 350, y: 210, width: 300, height: 40 },
    { id: "brain", label: "HERMES BRAIN & Graphify", role: "Plano de control persistente", detail: `${brain.memory.graphNodes} nodos AST · 1,115 relaciones`, state: status, kind: "control", stageId: "brain", x: 320, y: 262, width: 360, height: 46 },
    { id: "memory", label: "Memoria y Graphify", role: "Recuperación estructural", detail: "Análisis AST acotado por nivel L", state: brain.memory.state, kind: "memory", stageId: "routing", x: 60, y: 320, width: 230, height: 42 },
    { id: "router", label: "Model Router & VRAM", role: "Selección por capacidad", detail: `${brain.router.routes.length} rutas · Gemma / Agents-A1 / Qwen`, state: brain.router.state, kind: "router", stageId: "routing", x: 385, y: 320, width: 230, height: 42 },
    { id: "agents", label: "Agent Factory", role: "Especialistas reemplazables", detail: `${brain.agents.operatorCount} operador · ${brain.agents.advisoryCount} asesores`, state: brain.agents.state, kind: "factory", stageId: "routing", x: 710, y: 320, width: 230, height: 42 },
    { id: "atomic-plan", label: "Atomic Planner & Grafo DAG", role: "Descomposición Atómica", detail: "Tareas atómicas T-001..T-025 · Grafo DAG", state: status, kind: "policy", stageId: "atomic-plan", x: 320, y: 374, width: 360, height: 48 },
    { id: "execution-gateway", label: "Execution Gateway", role: "Control de ejecución", detail: "Allowlist, task ID y permisos", state: brain.sandbox.state, kind: "gateway", stageId: "plan", x: 350, y: 434, width: 300, height: 44 },
    { id: "code-sandbox", label: "Sandbox de código", role: "Worktree aislado", detail: `${brain.sandbox.active.length} tareas activas`, state: brain.sandbox.state, kind: "executor", stageId: "sandbox", x: 60, y: 490, width: 230, height: 44 },
    { id: "playwright", label: "Playwright MCP", role: "Validación en navegador", detail: "Navegador real bajo alcance MCP", state: brain.sandbox.state, kind: "executor", stageId: "sandbox", x: 385, y: 490, width: 230, height: 44 },
    { id: "automations", label: "Automatizaciones", role: "Operaciones controladas", detail: "Acciones explícitas y acotadas", state: brain.sandbox.state, kind: "executor", stageId: "sandbox", x: 710, y: 490, width: 230, height: 44 },
    { id: "evidence", label: "Patch Gate & Evidencia", role: "Inspección de parches", detail: "Diff hashes · limites lineas · evidencias", state: executionState, kind: "validation", stageId: "evidence", x: 330, y: 546, width: 340, height: 44 },
    { id: "integration", label: "Integración & Ensamblaje", role: "Ensamblaje coherente", detail: "Contratos · esquemas · compatibilidad", state: executionState, kind: "result", stageId: "integration", x: 350, y: 602, width: 300, height: 42 },
    { id: "validated", label: "Quality Gates Determinísticos", role: "Pruebas objetivas 100%", detail: "Sintaxis · Linters · PHPUnit · Tests", state: "healthy", kind: "result", stageId: "validated", x: 330, y: 656, width: 340, height: 42 },
    { id: "audit", label: "Auditoría Post-Ejecución", role: "Detección de errores", detail: "Requisitos · Seguridad · Datos · Regresión", state: "healthy", kind: "validation", stageId: "audit", x: 330, y: 710, width: 340, height: 42 },
    { id: "repair", label: "Repair Loop & Regresión", role: "Autocorrección de causas raíz", detail: "Fix T-Tasks · Max 5 estrategias · Re-tests", state: "healthy", kind: "validation", stageId: "repair", x: 330, y: 764, width: 340, height: 42 },
    { id: "learning", label: "Learning Engine Saneado", role: "Aprendizaje seguro", detail: `${brain.learning.counts.candidate ?? 0} candidatas saneadas`, state: brain.learning.state, kind: "learning", stageId: "learning", x: 350, y: 818, width: 300, height: 40 },
    { id: "promotion", label: "Promoción, Memoria & Entrega", role: "Entrega verificada", detail: "COMPLETED_AND_VERIFIED · Memoria", state: brain.learning.promotionState, kind: "learning", stageId: "promotion", x: 330, y: 870, width: 340, height: 40 },
    { id: "trama", label: "TRAMA", role: "Observador lateral", detail: "SSE read-only · Telemetría", state: "healthy", kind: "observer", stageId: "brain", x: 60, y: 262, width: 210, height: 46 },
  ];
  ...
  return {
    ...
    edges: [
      ...
      link("telegram", "auth", "Mensaje", "auth", telegramState),
      link("auth", "normalize", "Idempotente", "normalize"),
      link("normalize", "preclassify", "Especificación", "preclassify"),
      link("preclassify", "brain", "Complejidad L", "brain"),
      ...
      link("memory", "atomic-plan", "Contexto AST", "atomic-plan", brain.memory.state),
      link("router", "atomic-plan", "Capacidad", "atomic-plan", brain.router.state),
      link("agents", "atomic-plan", "Briefs", "atomic-plan", brain.agents.state),
      link("atomic-plan", "execution-gateway", "Tareas atómicas T-001", "plan"),
      link("execution-gateway", "code-sandbox", "Código", "sandbox", brain.sandbox.state),
      link("execution-gateway", "playwright", "Navegador MCP", "sandbox", brain.sandbox.state, "configured"),
      ...
      link("integration", "validated", "Integración", "validated", executionState),
      link("validated", "audit", "Quality Gates 100%", "audit", executionState),
      link("audit", "repair", "Hallazgos / Errores", "repair", executionState),
      link("repair", "atomic-plan", "Bucle de reparación", "atomic-plan", executionState),
      link("audit", "learning", "Sin errores críticos", "learning", brain.learning.state),
      link("learning", "promotion", "Saneado", "promotion", brain.learning.promotionState),
