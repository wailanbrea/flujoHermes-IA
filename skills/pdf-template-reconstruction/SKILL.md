...
metadata:
  hermes:
    tags: [laravel, blade, pdf, playwright, browsershot, puppeteer, html-css-print]
...
- **Plantillas clave**:
  - `resources\views\pdf\invoice.blade.php` (Factura / Informe de Intervención Técnica)
  - `resources\views\pdf\quotation.blade.php` (Presupuesto)
...
2. **Flujo de Reconstrucción Visual Iterativa (Desarrollo)**:
   - Imagen de referencia → Análisis de regiones → Especificación estructural → Blade + HTML + CSS → Render HTML → Generar PDF con Chromium → Convertir PDF a PNG → Comparar visualmente (Playwright/ImageMagick/MuPDF) → Corregir diferencias → Repetir (Máximo 8 iteraciones).
...
1. **No usar la imagen completa como fondo**.
2. **No convertir los textos en imágenes**.
3. **No calcular importes dentro de Blade** (subtotal, base imponible, impuesto, descuento, total, balance deben venir calculados del backend).
4. **No confiar en totales enviados desde web o Android**.
5. **No inventar propiedades de modelos** (Invoice, InvoiceItem, Currency, BankAccount, etc.).
6. **No cambiar controladores, servicios o migraciones sin necesidad**.
7. **No eliminar funciones existentes**.
8. **No romper la generación actual de facturas**.
9. **No mezclar factura y presupuesto en una plantilla llena de condicionales** (mantener archivos independientes `invoice.blade.php` y `quotation.blade.php`).
10. **No truncar productos o servicios silenciosamente**.
11. **No usar CDN, Google Fonts ni recursos externos** (usar fuentes e iconos locales/SVG).
12. **No usar JavaScript para calcular posiciones** (usar CSS de impresión).
13. **No usar Tailwind CDN**.
14. **No instalar dependencias sin presentar primero un plan y solicitar una sola aprobación**.
15. **Antes de modificar archivos, crear respaldo o rama Git**.
16. **Devolver archivos completos, no fragmentos incompletos**.
17. **Mantener actualizado el archivo `PDF_AGENT_TASKLIST.md`**.
18. **No entrar en un bucle infinito**: Máximo 8 iteraciones visuales. Si durante 3 iteraciones consecutivas no mejora el resultado, detenerse y entregar reporte con las diferencias.
19. **No declarar el trabajo terminado sin generar y revisar los PDFs reales**.
...
Especificación de página requerida:
```css
@page {
    size: A4 landscape;
    margin: 0;
...
- Unidades `mm` para dimensiones de página (`297mm x 210mm`).
...
A4 Landscape, print background, esperar carga de imágenes/fuentes locales, márgenes 0.
