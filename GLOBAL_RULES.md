...
14. REGLA DE LEVANTAMIENTO AUTÓNOMO DE SERVICIOS WEB Y XAMPP: Prohibido dar excusas o pedir pasos manuales al usuario. Detectar el stack y ejecutar autónomamente: PHP Vanilla (`php -S 127.0.0.1:8080`), Laravel (`php artisan serve`), Python (`python manage.py runserver` / `uvicorn`), Node/Vite (`npm run dev` / `dist`), validar la respuesta HTTP con `Invoke-WebRequest` y entregar la URL activa de inmediato.
